  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      const halfFloatTexType = isWebGL2
        ...
        : (halfFloat && (halfFloat as HalfFloatExtension).HALF_FLOAT_OES) || 0;

      let formatRGBA: FormatInfo = { internalFormat: 0, format: 0 };
      let formatRG: FormatInfo = { internalFormat: 0, format: 0 };
      let formatR: FormatInfo = { internalFormat: 0, format: 0 };

      if (isWebGL2) {
        const rgbaFormat = getSupportedFormat(
          gl,
          (gl as WebGL2RenderingContext).RGBA16F,
          gl.RGBA,
          halfFloatTexType
        );
        if (rgbaFormat) formatRGBA = rgbaFormat;

        const rgFormat = getSupportedFormat(
          gl,
          (gl as WebGL2RenderingContext).RG16F,
          (gl as WebGL2RenderingContext).RG,
          halfFloatTexType
        );
        if (rgFormat) formatRG = rgFormat;

        const rFormat = getSupportedFormat(
          gl,
          (gl as WebGL2RenderingContext).R16F,
          (gl as WebGL2RenderingContext).RED,
          halfFloatTexType
        );
        if (rFormat) formatR = rFormat;
      } else {
        const rgbaFormat = getSupportedFormat(gl, gl.RGBA, gl.RGBA, halfFloatTexType);
        if (rgbaFormat) formatRGBA = rgbaFormat;

        const rgFormat = getSupportedFormat(gl, gl.RGBA, gl.RGBA, halfFloatTexType);
        if (rgFormat) formatRG = rgFormat;

        const rFormat = getSupportedFormat(gl, gl.RGBA, gl.RGBA, halfFloatTexType);
        if (rFormat) formatR = rFormat;
      }
