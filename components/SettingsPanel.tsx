  ...
  ({ dictionary, className, ...rest }, ref) => {
    ...
    const handleLanguageChange = useCallback(
      (value: string) => {
        ...
        const newPath = currentPath.replace(/^\/[^/]*/, `/${CONFIG.languages[newLanguage].path}`);
    ...
    return (
      <div className={`flex flex-col w-full gap-4 ${className || ''}`} ref={ref} {...rest}>
        {/* Theme */}
        <div className="flex flex-col w-full px-4 pt-4 gap-2">
          <h3 className="text-md font-medium text-foreground">{dictionary.settings.theme.title}</h3>
          ...
          <Select value={selectedLanguage} onValueChange={handleLanguageChange}>
            <SelectTrigger className="w-full cursor-pointer">
              <div className="flex items-center gap-2">
                {selectedFlag}
                <SelectValue>
                  {dictionary.settings.language[CONFIG.languages[selectedLanguage].name]}
                </SelectValue>
              </div>
            </SelectTrigger>
            <SelectContent>
              {Object.keys(CONFIG.languages).map((lang) => (
                <SelectItem key={lang} value={lang} className="cursor-pointer">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span>
                      {dictionary.settings.language[CONFIG.languages[lang as Language].name]}
                    </span>
                  </div>
                </SelectItem>
              ))}
